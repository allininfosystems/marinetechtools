...
{var self=this;$H(data).each(function(item){var container=$('m2ePro_afn_qty_value_'+item.key);container.down('a').hide();container.down('.m2ePro-online-sku-value').remove();if(item.value!='empty'){container.down('.m2epro-afn-qty-data').show();container.down('.total span').update(item.value.total);container.down('.in-stock span').update(item.value.in_stock);}else{container.down('.m2epro-empty-afn-qty-data').show();}});}};});