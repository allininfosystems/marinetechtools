define(['jquery','M2ePro/Grid'],function(jQuery){window.EbayListingVariationProductManageVariationsGrid=Class.create(Grid,{initialize:function($super,gridId)
...
{var upc=form.down('.M2ePro-upc'),ean=form.down('.M2ePro-ean'),isbn=form.down('.M2ePro-isbn'),mpn=form.down('.M2ePro-mpn');upc.value=trim(upc.value);ean.value=trim(ean.value);isbn.value=trim(isbn.value.replace('-',''));mpn.value=trim(mpn.value);return jQuery(form).valid();}});});