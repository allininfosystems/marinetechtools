define(['M2ePro/Common'],function(){window.AmazonAccount=Class.create(Common,{initialize:function()
...
new Ajax.Request(url,{method:'post',parameters:Form.serialize($('edit_form')),onSuccess:function(transport){transport=transport.responseText.evalJSON();if(transport.success){window.close();}else{self.alert(transport.message);return;}}});},});});